...
class OutCosLayer(layers.Layer):
    def __init__(self, num_classes, seg=True, **kwargs):
        super(OutCosLayer, self).__init__(**kwargs)
        if num_classes == 2:
            self.num_classes = 1
        else:
            self.num_classes = num_classes
    ...
    def call(self, inputs, **kwargs):
        # return K.expand_dims(0.5*tf.norm(inputs, axis=-1), axis=-1)
        # return tf.divide(tf.add(inputs,1.0),2.0)#best but no ture
        return 1.0-tf.divide(tf.add(inputs,1.0),2.0)
    ...
    def get_config(self):
        config = {'num_classes': self.num_classes, 'seg': self.seg}
        base_config = super(OutCosLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class MultiplyLayer(layers.Layer):
    def __init__(self, num_classes=2, seg=True, **kwargs):
    ...
    def get_config(self):
        config = {'num_classes': self.num_classes, 'seg': self.seg}
        base_config = super(MultiplyLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class ReduceSumLayer(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        # return K.expand_dims(0.5*tf.norm(inputs, axis=-1), axis=-1) keep_dims=True
    ...
    def get_config(self):
        config = {'num_classes': self.num_classes, 'seg': self.seg}
        base_config = super(ReduceSumLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class DivideLayer(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        return tf.divide(inputs[0],inputs[1])
    ...
    def get_config(self):
        config = {'num_classes': self.num_classes, 'seg': self.seg}
        base_config = super(DivideLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class HalfLength(layers.Layer):
    def __init__(self, num_classes, seg=True, **kwargs):
        super(HalfLength, self).__init__(**kwargs)
        if num_classes == 2:
            self.num_classes = 1
        else:
            self.num_classes = num_classes
    ...
    def get_config(self):
        config = {'num_classes': self.num_classes, 'seg': self.seg}
        base_config = super(HalfLength, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
def update_routing(votes, biases, logit_shape, num_dims, input_dim, output_dim,
    ...
    def _body(i, logits, activations):
        ...
        route = tf.nn.softmax(logits, dim=-1)#对numcap求softmax
        preactivate_unrolled = route * votes_trans
        preact_trans = tf.transpose(preactivate_unrolled, r_t_shape)
        preactivate = tf.reduce_sum(preact_trans, axis=1) + biases#对inputcapsulenum求和
